...
function forgotPasswordToConfirmEmail(email) {
  ...
  return fetch(
    `${serverURL}/forgotPasswordToConfirmEmail`,
    requestOptions,
  ).then(handleResponse)
}
...
function forgotPassword(user) {
  ...
  return fetch(`${serverURL}/forgotPassword`, requestOptions).then(
    handleResponse,
  )
}
...
function emailVerify(email, verifyCode, password) {
  ...
  return fetch(
    `${serverURL}/users/verify?email=${email}&verifyCode=${verifyCode}&password=${password}`,
    requestOptions,
  ).then(handleResponse)
}
...
function getById(id) {
  ...
  return fetch(`${serverURL}/getUser/${id}`, requestOptions).then(
    handleResponse,
  )
}
...
function confirmCodeBeforeSignup(code) {
  ...
  return fetch(
    `${serverURL}/confirmCodeBeforeSignup?code=${code}`,
    requestOptions,
  ).then(handleResponse)
}
...
function update(user, file) {
  ...
  // formData.append('email', user.fullName)
  // formData.append("avarta", file)

  // console.log(user, file, '+++++++++++++++++++++++++++++++++++++++++++++++++++')
  // return axios.post(`${serverURL}/getUser/3`, formData, {
  //   headers: {
...
function updateForAdmin(user) {
  ...
  return fetch(`${serverURL}/admin/users/${user.id}`, requestOptions).then(
    handleResponse,
  )
}
...
function updateMfa(user) {
  ...
  return fetch(`${serverURL}/updateMfa/${user.id}`, requestOptions).then(
    handleResponse,
  )
}
...
function updatePassword(user) {
  ...
  return fetch(`${serverURL}/repassword/${user.id}`, requestOptions).then(
    handleResponse,
  )
}
...
function getLastPurchasesByDate(userId) {
  ...
  return fetch(`${serverURL}/lastPurchases/${userId}`, requestOptions).then(
    handleResponse,
  )
}
